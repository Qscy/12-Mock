...
class AssetManager:
    ...
    def __init__(self, data_dir, script_dir):
        ...
        self._inline_cache: Dict[str, tuple] = {}
    ...
    def read_cached(self, name: str) -> Optional[str]:
        """Return an asset's text, memoised until the file on disk changes."""
        ...
        try:
            ...
            return None
        with self._lock:
            hit = self._inline_cache.get(name)
        ...
        try:
            ...
            return None
        with self._lock:
            self._inline_cache[name] = (key, text)
        return text
...
def build_asset_tags(assets: AssetManager, vue_tag: Optional[str] = None) -> str:
    tags = [vue_tag if vue_tag else _asset_tag(assets, VUE_ASSET)]
...
def render_frontend(assets: AssetManager) -> str:
    # Vue is the one asset the UI cannot start without, so inline it whenever it
    # is on disk: the page then needs no second request for it, and a blocked or
    # stale asset URL can no longer blank the UI. (Vue's source contains no
    # "</script" or "<!--", so it is safe to embed verbatim.)
    vue_source = assets.read_cached(VUE_ASSET)
    vue_tag = f"<script>{vue_source}</script>" if vue_source else None
    return (FRONTEND_HTML
            .replace("<!--ASSET_TAGS-->", build_asset_tags(assets, vue_tag))
...
/* The fallback overlay must respect the `hidden` attribute: an inline
   `display:flex` would override it and cover the working UI on every load. */
#vue-missing:not([hidden]){display:flex;align-items:center;justify-content:center}
...
<div id="vue-missing" hidden style="position:fixed;inset:0;z-index:9999;background:#f1f5f9;padding:24px">
