...
def create_management_api(app, storage, mockjs, logger, auth_mgr, jwt_managers, psm: 'ProjectServerManager'):
    ...
    async def auth_status():
        return {"enabled": auth_mgr.enabled}
...
<div id="app" v-cloak>
<!-- Login -->
<div v-if="authOn&&!token" class="min-h-screen flex items-center justify-center">
...
<textarea class="ipt" v-model="resp._bodyText" placeholder='{"code":0,"data":{"name":"@cname"}}'
rows="6" @input="parseBody(resp)"></textarea></div>
...
<textarea class="ipt" v-model="ef.def.intercept._bodyText" rows="3" placeholder='{"error":"..."}'></textarea></div>
<!-- Redirect -->
...
<textarea class="ipt" v-model="jwt.extra" rows="2" placeholder='{}'></textarea></div>
...
const test=reactive({method:'GET',url:'',result:null});
...
try{const r=await fetch(base+test.url,{method:test.method});
